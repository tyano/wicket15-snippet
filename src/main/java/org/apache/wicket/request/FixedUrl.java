...
public final class FixedUrl implements Serializable
{
    ...
    private static final Pattern SCHEMA = Pattern.compile("([a-zA-Z]+):\\/\\/.*");
...
//		final int protocolAt = absoluteUrl.indexOf("://");
        Matcher m = SCHEMA.matcher(absoluteUrl);
		if (m.matches())
		{
            result.protocol = m.group(1).toLowerCase(Locale.US);;
//			result.protocol = absoluteUrl.substring(0, protocolAt).toLowerCase(Locale.US);

			final String afterProto = absoluteUrl.substring(result.protocol.length() + 3);
